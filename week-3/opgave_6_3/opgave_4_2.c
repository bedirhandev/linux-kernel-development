...
static int memdev_init(void)
{
	...
	if (mem_major) {
		...
		result = register_chrdev_region(devno, 1, MEMDEV_NAME);
	}
	else /* dynamically assigns the device number */
	{
		result = alloc_chrdev_region(&devno, 0, 1, MEMDEV_NAME);
...
static void memdev_exit(void)
{
	...
	unregister_chrdev_region(MKDEV(mem_major, 0), 1); /*release device number*/
