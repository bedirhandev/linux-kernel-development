...
static irqreturn_t mem_handler(int irq, void *dev_id)
{
	irq_counter++;
...
static int memdev_init(void)
{
	...
	// Reading about the use of the last argument in request_irq. Will come back at it.
	if(request_irq(irq, mem_handler, IRQF_SHARED, "my_interrupt", &my_dev_id)) 
	{
		printk(KERN_INFO "mem_irq: can't get assigned irq %i\n", 19);
		return -1;
	}

	printk(KERN_INFO "IRQ succesfully registered.");
